## 1.7.2
* Updates to make `analyzer` compatible with a newer version of `meta`
  that declares an extension on `TargetKind`. By using an override we
  make sure that the added extension does not cause a conflict.
  We also make sure that the `switch` in `get displayString` is always
  exhaustive by adding `default`.
